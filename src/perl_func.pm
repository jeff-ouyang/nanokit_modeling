...
BEGIN {
    ...
    @EXPORT = qw(&read_poscar &write_poscar &rotate_basis &read_othercar &write_othercar &dot_product 
                 ...
                 &set_bc &bbc &pbc &gauss &unit &middle_cal &file_org &triple_cal &triple_cal2);
   @EXPORT_OK = qw();
}
...
#----------------------------------------------------------------------
# subroutine middle_cal
#     This routine calculate the average positions of two POSCARS
#     
#     INPUT: $coo1 $coo2 $totatoms1
#     OUTPUT: $cool_middle: the average positions of two POSCARS
#----------------------------------------------------------------------
sub middle_cal 
{
    my $cool1 = shift;
    ...
    for($i=0; $i < $total_atoms; $i++){
        for($j=0; $j < 3; $j++){
            if (abs(($cool1->[$i][$j] - $cool2->[$i][$j]))>0.5){
                $cool_middle->[$i][$j] = (pbc($cool1->[$i][$j]) + pbc($cool2->[$i][$j])) / 2;         
            }
            else{
                $cool_middle->[$i][$j] = ($cool1->[$i][$j] + $cool2->[$i][$j]) / 2;
            }
        }
     }
     
     for($j=0; $j<3; $j++){
         $cool_middle->[$ind_atom-1][$j] = ($cool1->[$ind1-1][$j] + $cool1->[$ind2-1][$j]) / 2;
     }
     return ($cool_middle);
 }

#----------------------------------------------------------------------
# subroutine triple_cal
#     This routine calculate the average positions of two POSCARS
#     
#     INPUT: $coo1 $coo2 $totatoms1
#     OUTPUT: $cool_middle: the average positions of two POSCARS
...
sub triple_cal 
{
    ...
    for($i=0; $i < $total_atoms; $i++){
     ...
     for($j=0; $j<2; $j++){
         $cool_middle->[$ind_atom1-1][$j] = ($cool1->[$ind2-1][$j] - $cool1->[$ind1-1][$j]) / 3 + $cool1->[$ind1-1][$j];
     }
     $cool_middle->[$ind_atom1-1][2] = ($cool1->[0][2] + $cool1->[$total_atoms-1][2])/2;
     for($j=0; $j<2; $j++){
         $cool_middle->[$ind_atom2-1][$j] = ($cool1->[$ind2-1][$j] - $cool1->[$ind1-1][$j]) / 3 * 2 + $cool1->[$ind1-1][$j];
     }
     $cool_middle->[$ind_atom2-1][2] = ($cool1->[0][2] + $cool1->[$total_atoms-1][2])/2;
     return ($cool_middle);
 }
...
sub triple_cal2 
{
    ...
    for($i=0; $i < $total_atoms; $i++){
     ...
     for($j=0; $j<2; $j++){
         $temp1 = ($cool1->[$ind11-1][$j] + $cool1->[$ind12-1][$j]) / 2;
         $temp2 = ($cool1->[$ind21-1][$j] + $cool1->[$ind22-1][$j]) / 2;
         $cool_middle->[$ind_atom1-1][$j] = ($temp2 - $temp1) / 3 + $temp1;
     }
     $cool_middle->[$ind_atom1-1][2] = ($cool1->[0][2] + $cool1->[$total_atoms-1][2])/2;
     for($j=0; $j<2; $j++){
         $temp1 = ($cool1->[$ind11-1][$j] + $cool1->[$ind12-1][$j]) / 2;
         $temp2 = ($cool1->[$ind21-1][$j] + $cool1->[$ind22-1][$j]) / 2;
         $cool_middle->[$ind_atom2-1][$j] = ($temp2 - $temp1) / 3 * 2 + $temp1;
     }
     $cool_middle->[$ind_atom2-1][2] = ($cool1->[0][2] + $cool1->[$total_atoms-1][2])/2;
     return ($cool_middle);
 }
